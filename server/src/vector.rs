...
impl Engine {
    ...
    pub async fn add_points(&self, monuments: Vec<Monument>) -> Result<PointsOperationResponse, Box<dyn Error>> {
        let mut points = vec![];

        for (id, monument) in monuments.iter().enumerate() {
            let embeddings = self.generate_embedding(monument.description.as_str()).await?;
            points.push(
                PointStruct::new(id as u64, embeddings, [("id", monument.id.to_string().into())])
            )
        }
